...
const ITINERARY = [
  ...
  },
  {
    ...
    dayByDay: [
      ...
      { day: "Sep 19 (Sat)", plan: "🆕 THE BIG ZERMATT DAY — ski morning AND Gornergrat afternoon, both on the same day. ⛷️ Up early: Matterhorn Glacier Ride cable car to 3,883m — highest cable car in Europe, glass floor. Ski the Theodul Glacier ~08:00–13:00, ~21km of groomed piste at 3,000m+ with the Matterhorn beside you and Italy over the ridge. The glacier lifts close around 13:00 anyway, so the afternoon was always going to be free. Lunch at Chez Vrony (2,130m, Michelin-listed) on the way down. 🚂 Then GORNERGRAT — 33min cogwheel to 3,089m, Matterhorn dead ahead the whole way. Aim for the ~14:30 departure, back in the village by ~17:30. ⚠️ This is a long day on purpose: doing it here is what buys you a whole free afternoon in Grindelwald tomorrow." },
      { day: "Sep 20 (Sun)", plan: "🆕 EARLY START — this is now paragliding day. Check out and take the ~09:30 train, Zermatt→Visp→Interlaken Ost, arrive ~11:30. 🧳 Bags into the lockers at Interlaken Ost. 🪂 TANDEM PARAGLIDE FROM BEATENBERG (~12:00–14:00 including the minibus up): 20–30 min in the air over Lake Thun and Lake Brienz with the Eiger–Mönch–Jungfrau wall as the backdrop, landing back in Interlaken. This is the green-and-turquoise Switzerland shot. Quick lunch, collect bags ~14:30, train on to Grindelwald, check in ~15:15." }
    ],
    highlights: [
      ...
      { icon: "🚂", text: "Gornergrat railway — 33min cogwheel to 3,089m, Matterhorn head-on. Six times shorter than the Bernina, same wow. 🆕 Moved to Sat Sep 19 afternoon.", cost: "~CHF 88/person" },
  ...
  },
  {
    ...
    highlights: [
      ...
      { icon: "🎢", text: "🎢 DISNEY ADVENTURE WORLD (ex-Walt Disney Studios) + HOP — Fri Sep 25. Ratatouille and Crush's Coaster exist nowhere else on earth. ✅ CORRECTED: it does NOT close early — both parks run 09:30–21:00 in late September. It has no castle, and it is still mid-expansion. 🔴 VERIFY the attraction count for Sep 25 on disneylandparis.com before deciding whether it's a half day or a full one.", cost: "Part of the 2-day ticket + €20 hopper" },
    ...
    tips: [
      ...
      "✅ CORRECTION — THE SMALL PARK DOES NOT CLOSE EARLY. An earlier version of this plan said Walt Disney Studios shuts at 18:00–19:00 while Disneyland Park ran to 22:00. That was true for years and is no longer true: with the conversion to Disney Adventure World the hours were levelled up. The official calendar shows BOTH parks 09:30–22:40 in August and 09:30–21:00 on the late-September dates. Do not plan around the old assumption.",
      "🎡 THE TWO PARKS ARE STILL NOT EQUIVALENT, but for a narrower reason than hours. Disneyland Park has the castle, the parade, the nighttime castle show and ~15 headline rides. Adventure World has no castle and fewer rides — but it is mid-expansion, so 🔴 GO TO disneylandparis.com, PUT IN SEP 23 AND SEP 25, AND COUNT THE ATTRACTIONS EACH PARK LISTS. That date-picker shows attraction availability, and it settles half-day-versus-full-day better than any general rule. Whether Adventure World now has its own nighttime show is also worth checking there.",
...
const TICKETS = [
  {
    ...
    attractions: [
      ...
      { name: "Disneyland Paris — park hopper upgrade", price: "+~€20/person", note: "Worth it regardless of how the half-day question resolves — ✅ now that both parks run the same 09:30–21:00 hours, the hopper buys flexibility rather than rescuing a short day. It lets you finish Sep 25 in Disneyland Park for the castle show, which Adventure World has no equivalent of." },
...
const SPENT = {
  ...
  note: "Only the first two are confirmed figures. The three below them are genuinely committed money — they just have no number attached, so the 'confirmed' total is a floor, not a full picture of what you've already spent."
};
...
const BUDGET = {
  categories: [
    ...
    { label: "Intra-Europe transport (9 flights + 3 train legs)", min: 1653, max: 2253, note: "✅ THE AVE IS PAID: $243 for both Madrid↔Barcelona legs, so a $120–280 estimate became a fixed number (min +$123, max −$37). Remaining: the Eurostar and the London→Madrid flight are gone with London, replaced by one ORY→MAD hop (~$198 for 2) — Orly over CDG, 2h00 flight and metro line 14 straight there. Book the rest 2–3 months out." },
  ...
  total: { min: 13971, max: 18937, target: 14000 },
  savingTips: [
    ...
    "✅ $597 CONFIRMED PAID SO FAR: Barcelona Airbnb $272 + AVE $243 + Sagrada Família $82. All three landed inside or below their estimated ranges, and the Airbnb came in UNDER at $136/night against a $140–180 estimate. Replacing three guesses with three facts has narrowed the overall spread by ~$290. Keep recording every booking — the range only tightens.",
    "⚠️ HONEST WARNING: the midpoint is ~$16,500, about $2,500 over your $14,000 target. The two biggest single line items are the Cappadocia balloon ($440–620) and the Jules Verne dinner ($660–740, already committed). Neither is a mistake — but the target has not been met and pretending otherwise would not help you.",
...
const BOOKINGS = [
  ...
  { priority: 22, what: "Zermatt ski rental + Chez Vrony reservation (SAT Sep 19)", when: "2–3 weeks ahead", urgency: "medium", note: "Collect the rental the EVENING of Sep 18 — you arrive from Geneva in the late afternoon with the shops still open.", estimatedCost: "~CHF 50/person rental" },
  { priority: 23, what: "🆕 Madrid hotel near ATOCHA — 3 nights (Sep 26, 27, 28)", when: "1 month ahead", urgency: "medium", note: "🆕 UP FROM 2 NIGHTS, and the neighbourhood changed: Atocha, not the airport. You now land from Orly at a civilised hour instead of late at night, and Atocha is where the Toledo AVE leaves from. ⚠️ Segovia leaves from Chamartín instead — decide which day trip you want before booking.", estimatedCost: "$110–150/night" },
  { priority: 24, what: "Reconfirm Le Jules Verne (Sep 24, 8pm)", when: "Sep 22", urgency: "medium", note: "✅ Already booked, and it is the fixed point the entire itinerary was rebuilt around. Reconfirm 48h ahead and check the dress code — jacket required for men.", estimatedCost: "Already committed" },
  { priority: 25, what: "✅ DONE — Lima↔Madrid round-trip, Plus Ultra PNR XYJSDU", when: "✅ Booked", urgency: "medium", note: "Lima Sep 1 18:10 → Madrid Sep 2 12:45 (PU0302) · Madrid Sep 29 11:00 → Lima 16:10 (PU0301). Nothing to do but check in online 24h before each leg.", estimatedCost: "Already committed" }
];
...
const CROWD_DATA = {
  disneylandParis: {
    ...
    yourDates: "✅ WED SEP 23 (Disneyland Park, full day) + FRI SEP 25 (Studios, then hop). Wednesday is one of the two quietest days of the week in the quietest fortnight of the year — 15–30 min on the headliners — and it correctly gets the BIG park, which needs a full day and has the castle, the parade and the night show. Friday runs a little busier (25–40 min) as the weekend builds, but it gets Disney Adventure World, which has fewer headline rides, so the extra queue costs you less there. Then you hop back to Disneyland Park for the evening and the castle show. ✅ CORRECTED: both parks run 09:30–21:00 on these dates — the small park does NOT close early, as an earlier version of this plan claimed. 🔴 Rope drop at 09:30 both days: the first 90 minutes run 5–10 min waits on rides that hit 45+ by noon, and that is worth more than any paid Premier Access. ⚠️ The 08:30 'Extra Magic Time' is for Disney hotel guests only and does not apply to you."
  }
};
