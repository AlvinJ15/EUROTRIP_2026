...
const SPENT = {
  ...
  items: [
    ...
    { what: "🎡 Disney, Fri Sep 25 (2 pax) — the second Disney day", when: "Fri Sep 25", amount: null, note: "✅ BOOKED, amount not yet given. The plan estimated $194–248 for two on this day, so the confirmed total below is understated by roughly that much until you tell me. 🔴 Also worth confirming WHICH PARK: this was planned as Disney Adventure World — Sep 23 already bought Disneyland Park plus the reserved castle-show viewing, and the two nighttime shows run simultaneously, which is the whole reason the second day exists. A second Disneyland Park ticket would repeat the show and skip Frozen Ever After, Ratatouille and Avengers Campus." },
  ],
  note: "⚠️ One booking is committed without a figure — the Sep 25 Disney tickets. The confirmed total below is therefore a FLOOR, understated by roughly $194–248, not a complete picture. Send me the amount and it moves up."
};
...
const BUDGET = {
  ...
  total: { min: 14554, max: 18692, target: 15000 },
  savingTips: [
...
const BOOKINGS = [
  ...
  { priority: 12, what: "✅ DONE — Disney tickets, FRI SEP 25 (2nd Disney day)", when: "✅ BOOKED", urgency: "low", note: "✅ BOUGHT. Both Disney days are ticketed and the Paris block now has nothing left that can sell out. 🔴 TWO THINGS STILL OPEN, AND THEY ARE NOT THE SAME QUESTION: (1) the AMOUNT — the spend tracker carries this as committed-but-unpriced, so the confirmed total is a floor until you send it; (2) WHICH PARK. The plan assumed DISNEY ADVENTURE WORLD, because Sep 23 already bought Disneyland Park plus the reserved castle-show viewing, and the two nighttime shows run SIMULTANEOUSLY — that simultaneity is the entire reason a second day exists. A second Disneyland Park ticket would repeat the same show and skip Frozen Ever After, Ratatouille, Avengers Campus and Cascade of Lights. ⏰ If it is Adventure World: rope drop 09:30, Frozen Ever After first, Crush's Coaster second. ❌ Still no hopper and no Premier Access Ultimate (€110–230/person); decide a single-ride Premier Access for Frozen that morning in the app.", estimatedCost: "✅ booked — amount not yet recorded" },
