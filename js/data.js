...
const ITINERARY = [
  ...
  },
  {
    ...
    tips: [
      ...
      "⛪ ST. PETER'S BASILICA IS NOT PART OF THE MUSEUMS — free entry, separate door, and it stays in the plan. Do the €10 dome climb: 551 steps, and it is the best view in the city. Dress code: covered shoulders and knees, enforced. 🆕 AND THE HOTEL BOOKING MAKES THIS THE FIRST THING YOU DO ON SEP 17: you are a 10-minute walk away, so be at the door for the 07:00 opening. The security queue reaches an hour or more by mid-morning, and arriving at 11:00 like everyone else wastes the one advantage the location gives you. Basilica ~07:15, dome from ~08:00, down by ~09:00, then metro A from Ottaviano to the Colosseum — which means booking the Colosseum VR slot for ~10:00 rather than the earliest of the day.",
      ...
      "Colosseum: book the guided tour WITH the VR add-on at ancientandrecent.com — you asked for the immersive stuff and this is the best of it. 🔄 REVISED SLOT: this used to say take the earliest of the day, which was right when the hotel was going to be central. Now that you are next to St Peter's, take ~10:00 instead and put the basilica and the dome in front of it — St Peter's is the one where being early actually saves you an hour of queue, and the Colosseum is a timed ticket that does not reward it the same way.",
...
const SPENT = {
  ...
  items: [
    ...
    { what: "🏛️ Rome hotel, 2 nights — ~10 min walk from the Vatican", when: "Wed Sep 16 → Fri Sep 18", amount: 308, note: "✅ $154/night against a $140–190 estimate — INSIDE PLAN and near the bottom of it, which is worth noting after Grindelwald broke the same kind of estimate twice. 🔄 NOT the Trastevere this plan recommended, and that is fine, but it reorders Sep 17. ⛪ ST PETER'S BASILICA IS NOW 10 MINUTES ON FOOT, and it is free and entirely separate from the Vatican Museums that were cut. So it moves to FIRST thing: at the door for the 07:00 opening, dome climb from ~08:00, down by ~09:00, then metro A across to a ~10:00 Colosseum slot. The security queue at St Peter's passes an hour by mid-morning, so the location only pays if you use it early — the Colosseum is a timed ticket and does not reward earliness the same way. ⚠️ THE TRADE: Trastevere dinner is now a ~25 min walk rather than downstairs, and the Colosseum, Forum and Pantheon are 25–40 min out. ✈️ And the plan's 'Leonardo Express from Termini' on Sep 18 assumed a central hotel — from here that is a backtrack, so confirm whether Rome's fixed €50 airport taxi fare reaches your address (it applies inside the Aurelian Walls; Prati/Vatican is outside) before relying on it." },
  ],
...
const BUDGET = {
  ...
  total: { min: 15100, max: 18794, target: 15000 },
  savingTips: [
  ...
  hotelVsAirbnb: [
    ...
    { city: "Rome", nights: 2, firstNight: 16, recommendation: "✅ BOOKED — $308, ~10 min walk from the Vatican", reason: "✅ PAID: $308 for both nights, $154/night against a $140–190 estimate — the first booking in a while to land inside plan, and near the bottom of it. 🔄 NOT the Trastevere this plan suggested, and the swap is fine — but it changes Sep 17, so read this. ⛪ WHAT IT BUYS: ST PETER'S BASILICA IS 10 MINUTES ON FOOT. The Basilica is free and is NOT part of the removed Vatican Museums, so it was always in the plan — and now you can be at the door when it opens at 07:00, before the security queue builds to the hour-plus it reaches by mid-morning. 🔴 SO DO ST PETER'S FIRST ON SEP 17, not the Colosseum: basilica ~07:15, dome climb from ~08:00 (€10, 551 steps), down by ~09:00, then metro A from Ottaviano across to the Colosseum. Being 10 minutes away is worth nothing if you go at 11:00 like everyone else. 👗 Dress code enforced — covered shoulders and knees. ⚠️ WHAT IT COSTS: Trastevere is now a ~25 min walk or a bus rather than downstairs, so the evening you eat there is a deliberate trip, not a stroll. And the Colosseum, Forum and Pantheon are all 25–40 min away rather than close. ✈️ SEP 18 AIRPORT: the plan's 'Leonardo Express from Termini' assumed a central hotel — from the Vatican that is a backtrack. Check whether Rome's €50 fixed taxi fare to FCO covers your exact address (it is defined as inside the Aurelian Walls, and Prati/Vatican sits outside, so confirm rather than assume) and if not, agree the fare before getting in or allow the extra time to reach Termini.", cost: "✅ $308 for both nights ($154/night), paid" },
