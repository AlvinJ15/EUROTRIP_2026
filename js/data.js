...
const ITINERARY = [
  ...
  },
  {
    ...
    dailyCost: 340,
    accommodation: "🆕 TWO VILLAGES, THREE HOTELS: Grindelwald Sep 18 + 19 (arrival and the Spinne), then INTERLAKEN Sep 20 + 21. The split is priced, not aesthetic — Grindelwald wanted ~$800 for those last two nights against ~$400 in Interlaken, and Interlaken is closer to everything those two days do.",
    ...
    tips: [
      ...
      "🏨 🆕 TWO BASES, AND THE SECOND ONE SAVES ~$400. Grindelwald Sep 18–20, Interlaken Sep 20–22. 🔴 THIS REVERSES EARLIER ADVICE IN THIS PLAN, which said to keep all four nights in Grindelwald because moving them would save only $40–80. Real quotes came in at ~$800 for the two nights in Grindelwald against ~$400 in Interlaken for comparable private rooms, and the cheap Grindelwald listings were hostels and shared rooms. ✅ THE FIRST TWO NIGHTS STAY IN GRINDELWALD AND ARE NOT NEGOTIABLE: the Spinne is paid, and Sep 19 starts on the 08:00 First gondola, which only works from the village. ✅ THE LAST TWO GAIN TIME AS WELL AS MONEY — Sep 20 is the Interlaken day, Sep 21's Lauterbrunnen is 20 min from Interlaken Ost against 40 from Grindelwald, and on Sep 22 the GoldenPass leaves from the station you slept next to. 🧳 One bag-move, on the morning of Sep 20, on a train you were taking regardless.",
...
const BUDGET = {
  ...
  total: { min: 14504, max: 18358, target: 15000 },
  savingTips: [
  ...
  hotelVsAirbnb: [
    ...
    { city: "🆕 Interlaken — nights 3–4 (Sep 20→22)", nights: 2, firstNight: 20, recommendation: "🆕 MOVED OUT OF GRINDELWALD — a decent room near Interlaken Ost", reason: "🆕 THE PRICES DECIDED THIS, NOT THE MAP. Real quotes for these two nights came back at ~$800 in Grindelwald against ~$400 in Interlaken, for comparable private rooms — the cheap Grindelwald options were hostels and shared rooms, which are not on the table. An earlier version of this plan estimated the gap at $40–80 and recommended staying put; the estimate was wrong by a factor of five and the recommendation went with it. ✅ AND THE LOGISTICS AGREE, which is what makes this easy: Sep 20 IS the Interlaken day, so the bags ride the train you were taking anyway instead of walking to a third Grindelwald guesthouse. Sep 21 goes to Lauterbrunnen — 20 min direct from Interlaken Ost against ~40 min from Grindelwald with a change at Zweilütschinen, so you get ~40 min a day back. Sep 22 the GoldenPass leaves FROM Interlaken Ost at 11:07 and you are already standing there, no 09:37 train with luggage. ❌ WHAT YOU GIVE UP, and it is real: the Eiger north face out of the window and the alpenglow on it at sunset. You still have that on Sep 18 and from the Spinne on Sep 19 — the two nights that were always going to be the views. Interlaken is a town between two lakes, not an alpine village. 🔴 Book near INTERLAKEN OST, not Interlaken West: Ost is the station every one of these days uses.", cost: "$100–140/night" },
...
const BOOKINGS = [
  ...
  { priority: 23, what: "🏨 Grindelwald guesthouse (Sep 18→19) + 🆕 INTERLAKEN hotel (Sep 20→22)", when: "🔴 NOW", urgency: "high", note: "🆕 THE SECOND ONE MOVED TO INTERLAKEN, ON PRICE. Grindelwald quoted ~$800 for those two nights against ~$400 in Interlaken for comparable private rooms; the sub-$400 Grindelwald listings were hostels and shared rooms, which are ruled out. ✅ SEP 18 STAYS IN GRINDELWALD — a ~15:15 arrival and an 08:00 Firstbahn the next morning both need the village, and this is one night so the premium is small. Stay near the station. 🆕 SEP 20→22 IN INTERLAKEN, near INTERLAKEN OST (not West): Sep 20 is the Interlaken day, Sep 21's Lauterbrunnen is 20 min away instead of 40, and Sep 22's GoldenPass departs from that station. 🔴 Ask the Interlaken hotel whether it holds luggage before check-in — your bags arrive on the morning of Sep 20. If it will not, Interlaken Ost has lockers, so it is not a dealbreaker.", estimatedCost: "$150–240 for Sep 18 + $100–140/night × 2" },
